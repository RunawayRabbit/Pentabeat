...
float UMidiComponent::playFromMarker(const FString Marker)
{
	MidiTrack* track = mMidiFile->getTracks().at(0);

	for(const auto& Event : track->getEvents())
	{
		if(Event->getType() == MetaEvent::MARKER)
		{
			TextualMetaEvent* textExEvent = static_cast<TextualMetaEvent*>(Event);
			FString Text = UTF8_TO_TCHAR(textExEvent->getText().c_str());
			if(Text == Marker)
			{
				return mProcessor.startFromTick(Event->getTick());
			}
		}
	}
